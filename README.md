...
Nano-Floor Designer brings AI-powered flooring visualizations directly into WordPress. Upload a room image, choose a flooring preset (or craft a custom description), and let Google's Gemini 3.1 Flash Image Preview model generate a realistic preview. The plugin supports both shortcodes and a Gutenberg block so the experience can live anywhere on your site.
...
- Settings page for Gemini credentials (defaults to gemini-3.1-pro-preview and gemini-3.1-flash-image-preview), rate limiting, and debug logging
